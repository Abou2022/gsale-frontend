const validateAttendee = attendee => {
    if (!attendee.profile_id || !attendee.garageSaleEvent_id) {
        throw new Error('VALIDATION ERROR: attendee requires a profile_id, and garageSaleEvent_id.');
    }
};
...
let attendees = (state = [], action) => {
    ...
    switch (type) {
        ...
        case 'ATTENDEE_DELETE':
            if (state === []) throw new Error('USAGE ERROR: can not delete attendee not in state');
            validateAttendee(payload);

...
export const vendorFetchRequest = vendorID => (dispatch) => {
    return superagent.get(`${process.env.REACT_APP_API_URL}/api/vendors/${vendorID}`)
        .then(res => {
...
export const vendorsFetchRequest = () => (dispatch) => {
    return superagent.get(`${process.env.REACT_APP_API_URL}/api/vendors`)
        .then(res => {
...
export const vendorCreateRequest = vendor => (dispatch) => {
    ...
    return superagent.post(`${process.env.REACT_APP_API_URL}/api/vendors`)
...
export const vendorDeleteRequest = vendorId => (dispatch) => {
    const token = localStorage.getItem("gSaleToken");
    return superagent.delete(`${process.env.REACT_APP_API_URL}/api/vendors/${vendorId}`)
        .set('Authorization', `Bearer ${token}`)
        .then(res => {
...
export const vendorUpdateRequest = vendor => (dispatch) => {
    const token = localStorage.getItem("gSaleToken");
    return superagent.put(`${process.env.REACT_APP_API_URL}/api/vendors/${vendor.id}`)
        .set('Authorization', `Bearer ${token}`)
        .send(vendor)
        .then(res => {

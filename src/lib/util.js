...
export const getZipsArray = async (lat, lng, radius = 10) => {
    try {
        ...
        let zips = JSON.parse(localStorage.getItem(`gSaleZips-lat-${lat}-lng-${lng}-rad-${radius}`));
        if (zips) {
            return zips;
        }
        const res = await superagent.get(`https://www.freemaptools.com/ajax/us/get-all-zip-codes-inside.php?radius=${radius}&lat=${lat}&lng=${lng}&rn=563&showPOboxes=false`);
        let arr = res.text.split('<postcode postcode="');
        zips = [];
        for (let i = 1; i < arr.length; i++) {
            zips.push(arr[i].substring(0, 5));
        }
        localStorage.setItem(`gSaleZips-lat-${lat}-lng-${lng}-rad-${radius}`, JSON.stringify(zips));
        return zips;
    } catch (err) {
